...
import { Modal as ModalAntd , Button as ButtonAntd,InputNumber  } from 'antd';
import {
...
const CalendarEventInfo = (props) => {
    ...
    const [linkRegistrer, setLinkRegistrer] = useState('');
    const [isLoadingLink, setIsLoadingLink] = useState(false);
    const [userId , setUserId] = useState(null);
    const [modalNumberMember , setModalNumberMember] = useState(false);
    const [members , setMembers] = useState(1);
    const [isMentor , setIsMentor] = useState(false);
    const [isEntidad , setIsEntidad] = useState(false);
    const {get,post} = useFetch(URL_BASE)
    ...
    useEffect(() => {
        (async ()=>{
            ...
            let api = `${URL_BASE}/usuario/link/existe/${event.id}`
            ...
            data[0] ? setLinkRegistrer(data[0]) : setLinkRegistrer('')
          
        })();
        
        
        setIsMentor(localStorage.getItem('mentor') === 'true')
        setIsEntidad(localStorage.getItem('entidad') === 'true')
    })
...
const copyToClipboard = async () => {
    await navigator.clipboard.writeText(linkRegistrer)
    toast.success('Link copiado al portapapeles')

}
const handleRegistrerLink = async () => {
    setModalNumberMember(false)
    let body  = {
        "creador":userId,
        "entidad":event.entidad,
        "evento":event.id,
        "fecha_expiracion":event?.fecha_fin,
        "maximo_usuarios":members,
    ...
    const response = await post(`/usuario/link/crear`, body)
    if (response) {
        setLinkRegistrer(response.url)
        setLoading(false);
...
const handleCancel = () => {
    ...
    return (
        ...
        <Row justify='center' id="calendar-event-info">
                                    ...
                                    <Row>
                                        <Col xl={24} lg={24} md={24} sm={24} xs={24}>
                                            ...
                                            <Row>
                                                ...
                                                {/* colTimeZonetoOtherTimeZone
                                                colTimeZonetoOtherTimeZone(event.inicio,timeZone.value )
                                                */}
                                                <Col xl={12} lg={12} md={12} sm={11} xs={11}>
                                                    <p><strong>Fecha - Hora  {timeZoneSelect.short}</strong></p>
                                                    <p><Moment format="ddd MMMM D YYYY" utc
                                                               local>{moment(colTimeZonetoOtherTimeZone(event.inicio,timeZoneSelect.value )).local('America/Bogota')}</Moment>
                                                    </p>
                                                    <p><Moment format="h:mm A z" utc
                                                               local>{moment(colTimeZonetoOtherTimeZone(event.inicio,timeZoneSelect.value )).local('America/Bogota')}</Moment> - <Moment
                                                        format="h:mm A z" utc
                                                        local>{moment(colTimeZonetoOtherTimeZone(event.fin,timeZoneSelect.value )).local('America/Bogota')}</Moment></p>
                                                </Col>
                                            </Row>
                                            <Divider style={{marginBottom: '2%', marginTop: '0%'}}/>
                                            <Row>
                    ...
                    <Col>
                                                    {
                                                        (isMentor || isEntidad) ?     
                                                            ...
                                                            isLoadingLink ?  <Button className='btn-verde-basico'
                                                            disabled={true}>
                                                                <div>
                                                                <Spin>
                                                              Link...
                                                            ...
                                                            :   linkRegistrer ? 
                                                            //componente para copiar en el portapapeles el link de registro
            ...
            <ModalAntd title="Cantidad de miembros" visible={modalNumberMember} okText="Crear" onOk={handleRegistrerLink} onCancel={handleCancel}>
            <Row>
                <Col>
               <span>Cantidad de usuarios que se pueden registrar</span> <InputNumber min={1} max={300} defaultValue={1} onChange={onChangeInputNumber} />
                </Col>
            </Row>
