...
const ModalReviews = (props) => {
    ...
    const handleSubmit = async () => {
        ...
        if(!goal_reached && !goal_reachedNo){
            toast.error('Por favor marque si se cumplió o no el objetivo de la sesión')
            return
        }
        if(review===""){
        ...
        let request = await fetch(url, {
            ...
            method: 'POST',
            body: JSON.stringify({'puntaje': calificacion, 'comentario': review, 'goal_reached': goal_reached ? true: false}),
        })
    ...
    return (
        <div>
            <Modal isOpen={modal} className={className} show={show} toggle={toggle}>
                <ModalHeader id='titulo-modales-blancos'><h5><strong>Por favor califica tu sesión:</strong></h5>
                </ModalHeader>
                <ModalBody>
                    ...
                    <Input style={{color: 'black'}} id="text-area-reviews" type="textarea"
                           placeholder='Por favor déjanos tus comentarios' rows={5}
                           onChange={(obj) => {
                ...
                <ModalFooter>
                    <Button className='btn-verde-basico' onClick={handleSubmit} type="submit">{loading? <Spin size='small'/>: 'Enviar'}</Button>{' '}
