...
const EventEditForm= (props) => {
    ...
    const [inicio, setInicio] = useState(moment(event?.fecha_inicio, 'YYYY-MM-DD HH:mm')._i)
    const [fin, setFin] = useState(moment(event?.fecha_fin, 'YYYY-MM-DD HH:mm')._i)
    ...
    const filter = (e) => {
        console.log(users)
         //crear una expresion regular que elimine todos los espacios en blanco y cree un elemto de tipo array por cada ","
         let array = e.target.value.split(",").map(function (item) {
             return item.trim();
         });
                        ...
                        <Col lg={24} md={24} sm={24} xs={24} style={{marginLeft:'0%'}} offset={isMobile ? 0 : 6}>
                            {
                                (type === 'Grupal' || type === 'Junta Directiva') && (
                                    ...
                                    >
                                        ...
                                        <Input  onChange={(e) => filter (e)} placeholder='Seleccione tema'/>
                                    </Form.Item>
                                    </>
                                )
                            }
