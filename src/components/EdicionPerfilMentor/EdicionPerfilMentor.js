...
useEffect(() => {
/**
 * It fetches a user's data from an API, and if the user has a Google Calendar, it sets the state of a
 * boolean to true.
 * </code>
 */
    async function getCalendatsync () {

    try {
        const request = await fetch(`${URL_BASE}/usuario/${props.username}`, {
            method: 'GET',
            headers: new Headers({
        ...
        console.log(json)
        if(   json[0].api_calendario === "gcalendar" ){
            setIsGoogle(true)
        }else{
            setIsManual(true)
        }
...
getCalendatsync()
    ...
    useEffect(() => {
        ...
        const obtainPaises = async () => {
            const listaPaises = [{name:"Colombia"},{
                name:"Argentina"
            },
        {
            name:"Mexico"
            ...
            >
                <Row gutter={{xs: 8, sm: 16, md: 24, lg: 32}}>
                    <Col lg={24} md={24} sm={24} xs={24}>
                        ...
                        <Row>
                            <Col lg={11} md={11} sm={24} xs={24}>
                            <Form.Item name="sync" label="Sinc calendario: ">
                              <div className="check-box-container">
                      
                                  <Checkbox checked={isGoogle} onChange={ () => onChange("google")}>
                                <img className='google-image' width={200} src={process.env.PUBLIC_URL + '/static/google.png'}/>
                                  </Checkbox>
                                  <Checkbox checked={isManual} onChange={ () => onChange('manualmente')} className='checkbox-2'>
                                  Manualmente
                                  </Checkbox>
