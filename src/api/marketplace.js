...
export async function getDispoMentor(id,date) {
    // ${URL_BASE}/usuario/synccalendar/sync/${mentor}/${date}
...
export async function getDispoMentorDay(date, mentor, entidad) {
    ...
    if (entidad) {
        ...
        url = `${URL_BASE}/usuario/synccalendar/sync/${mentor}/${date}`
    }
    try {
...
export const createEventFromUserNormal = async ({
    ...
    try {
        // https://api-desarrollo.grovity.co/calendario/crear/evento/dispo
        // ${URL_MARKETPLACE}/entity/events`
        console.log(`${URL_BASE}calendario/crear/evento/dispo`)
        const request = await fetch(`${URL_BASE}/calendario/crear/evento/dispo`, {
            method: 'POST',
            headers: new Headers({
                'Authorization': `Token ${localStorage.getItem("token")}`
            }),
