...
export async function getFilesEventsAll(username) {
    ...
    let urls = await request.json()
    if (urls?.detail) window.location.href = window.location.origin + "/login";
    return {
        'archivos': archivos,
        'urls': urls,
    };
...
export async function getEventbyId(id, bool) {
    ...
    if (requestJson?.detail) window.location.href = window.location.origin + "/login";
    if (requestJson.status === 401) {
        console.log("token expirado")
        toast.error(requestJson.message);
        // go to login page
